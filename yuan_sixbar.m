...
A=1; B=4; C=1.5; D=4; E=0.5; F=2; G=4.5; H=2;

t = 0:0.05:10;  % Time series
...
set(gca,'xlim',[-5 7],'ylim',[-3 7]);
title(animation,'Animation and Trace of P6');
    
dis = subplot(2,2,2);
set(dis,'xlim',[0 max(theta)/pi*180],'ylim',[0 5]);
xlabel(dis, 'Input Angle \theta (\circ)');
ylabel(dis, 'Amplitude (m)');
title(dis,'Displacement');
grid on;

vel = subplot(2,2,3);
set(vel,'xlim',[0 max(theta)/pi*180],'ylim',[0 2]);
...
acc = subplot(2,2,4);
set(acc,'xlim',[0 max(theta)/pi*180],'ylim',[-2 2]);
...
for i=1:length(theta);
    ...
    str6 = ['Input \theta=',num2str(theta(i)/pi*180),'\circ'];
    P2_text = text(P2(1,i),P2(2,i)+0.4,str1);
    P3_text = text(P3(1,i),P3(2,i)+0.4,str2);
    P5_text = text(P5(1,i),P5(2,i)-0.4,str3);
    P6_text = text(P6(1,i),P6(2,i)+0.4,str4);
    P7_text = text(P7(1,i),P7(2,i)+0.4,str5);
    angle_text=text(-2,6,str6);
    ...
    dis = subplot(2,2,2);
    hold on;
    plot(dis,theta(1:i)/pi*180,P2_d(1:i),'r');
    plot(dis,theta(1:i)/pi*180,P3_d(1:i),'b');
    plot(dis,theta(1:i)/pi*180,P5_d(1:i),'g');
    plot(dis,theta(1:i)/pi*180,P6_d(1:i),'m');
    plot(dis,theta(1:i)/pi*180,P7_d(1:i),'k');
    hold off;
    %%
    if i<length(theta)
...
%         delete(P6_circle);
        ...
        vel = subplot(2,2,3);
        hold on;
        plot(vel,theta(1:i)/pi*180,P2_v(1:i),'r');
        plot(vel,theta(1:i)/pi*180,P3_v(1:i),'b');
        plot(vel,theta(1:i)/pi*180,P5_v(1:i),'g');
        plot(vel,theta(1:i)/pi*180,P6_v(1:i),'m');
        plot(vel,theta(1:i)/pi*180,P7_v(1:i),'k');
        hold off;
        %%
        if i<length(theta)-1
            ...
            acc = subplot(2,2,4);
            hold on;
            plot(acc,theta(1:i)/pi*180,P2_a(1:i),'r');
            plot(acc,theta(1:i)/pi*180,P3_a(1:i),'b');
            plot(acc,theta(1:i)/pi*180,P5_a(1:i),'g');
            plot(acc,theta(1:i)/pi*180,P6_a(1:i),'m');
            plot(acc,theta(1:i)/pi*180,P7_a(1:i),'k');
            hold off;
        end
    end
end
